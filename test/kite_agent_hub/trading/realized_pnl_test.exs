defmodule KiteAgentHub.Trading.RealizedPnlTest do
  ...
  describe "compute_realized_pnl_for_sell/1" do
    ...
    test "cross-agent isolation — buys from another agent are NOT consumed", %{
      user: user,
      agent: agent
    } do
      ...
      _ =
        insert_settled_trade!(%{user: other_user, agent: other_agent}, %{
          action: "buy",
          contracts: 100,
    ...
    test "cross-market isolation — buys on another market are NOT consumed", %{
      user: user,
      agent: agent
    } do
      _ =
