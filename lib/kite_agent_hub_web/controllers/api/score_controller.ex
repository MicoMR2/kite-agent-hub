defmodule KiteAgentHubWeb.API.ScoreController do
  ...
  Batch is capped at 10 tickers per request (Phorari PR #2 scope —
  swing-strategy candidate sweeps rarely need more) so a single caller
  cannot exhaust the Alpaca data-tier quota (200 req/min free tier).
  The controller fans out bars fetches via `Task.async_stream` with
  `max_concurrency: 5` — `AlpacaClient.bars/5` already honors
  Retry-After on 429, so a temporary burst backpressures cleanly
