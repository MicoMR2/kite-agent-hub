defmodule KiteAgentHub.Accounts do
  @moduledoc """
  ...
  alias KiteAgentHub.Accounts.{User, UserToken, UserNotifier}
  ...
  See `KiteAgentHub.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `KiteAgentHub.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  def login_user_by_magic_link(token) do
    ...
    case Repo.one(query) do
      # Password-registered user clicking their confirmation link.
      # Confirm the email only — do not issue a session (session-fixation guard).
      # Return {:confirmed, user} so the controller can redirect to the password login page.
      {%User{confirmed_at: nil, hashed_password: hash} = user, _token} when not is_nil(hash) ->
        with {:ok, {confirmed_user, _}} <-
               user |> User.confirm_changeset() |> update_user_and_delete_all_tokens() do
          {:confirmed, confirmed_user}
        end
